...
from math import cos,sin,tan,floor,sqrt,pi
...
def Geo2UTM(PuntoGeodesico,NombreElipsoide,Huso=None,w=True,kp=True):
    ...
    if Huso==None:
        ...
        if Lon<lon0i or Lon>lon0d:
            raise Exception("Solo se pueden forzar las coordenadas a los Husos adyacentes un máximo de 20', si estas se en cuentran en el extremo del Huso")
...
def Geo2UTMFromFile(File,NombreElipsoide,w=True,kp=True,parent=None):
    ...
    if not type(File)==str:
        raise Exception("El formato del fichero introducido no es de tipo str.")
    if not exists(File):
        raise Exception("El fichero introducido no existe.")
    if not isfile(File):
        raise Exception("El fichero introducido no se corresponde con un archivo.")
